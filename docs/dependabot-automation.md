...
sequenceDiagram
  ...
  Actions->>Actions: lint test and test-e2e jobs
...
- Required checks (job ids; the kubebuilder `Run on Ubuntu` display names were removed so these are unique):
  - `lint` from [Lint](../.github/workflows/lint.yml)
  - `test` from [Tests](../.github/workflows/test.yml)
  - `test-e2e` from [E2E Tests](../.github/workflows/test-e2e.yml)
- If branch protection still requires `Run on Ubuntu`, replace it with the three names above **before** this workflow lands on `master`. Otherwise auto-merge waits forever (old check name never reports) or, if that required check is removed and nothing replaces it, PRs can merge with no CI.
...
1. Point required checks at `lint`, `test`, and `test-e2e` (remove `Run on Ubuntu` if it was selected).
2. Confirm App install, `APP_CLIENT_ID` / `APP_PRIVATE_KEY` in both secret stores, auto-merge, and squash are already configured.
3. Merge this change into `master` last so a minor/patch Dependabot PR cannot merge before tests finish.

## Verify

1. Minor or patch Dependabot PR: App approval, auto-merge queued, squash merge after `lint`, `test`, and `test-e2e` are green.
